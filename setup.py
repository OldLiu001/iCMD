...
from iCMD import __version__
from iCMD.kernel import CMDKernel
...
def is_kernel_spec_installed() -> bool:
    try:
        KernelSpecManager().get_kernel_spec(CMDKernel.language)
        return True
    except NoSuchKernel:
        return False
...
def install_kernel_spec():
    ...
    KernelSpecManager().install_kernel_spec(default_spec_dir, CMDKernel.language, prefix=sys.prefix)
...
setup(
    ...
    description="CMD Jupyter Kernel",
    classifiers=[
    ...
    entry_points={
        'console_scripts': ['iCMD=iCMD.app:main']
    },
