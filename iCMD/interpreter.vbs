...
Class Interpreter
    ...
    Public Sub Run()
        Dim ocmd
        set ocmd = CreateObject("WScript.Shell").Exec("cmd /k echo off")
        'ocmd.StdIn.WriteLine "echo %IVBS_RET_PATH% & exit"
        'msgbox ocmd.StdOut.ReadAll()
		ocmd.stdout.close()
        On Error Resume Next
		...
		stderr = ""
            ...
            cmd = RTrim(cmd)
            'msgbox cmd'input
            'WSH.Echo 2333'output
            'msgbox retFilePath'random
            ocmd.StdIn.WriteLine "("
	 ocmd.stdin.writeline ""+ cmd & ""
	    ocmd.stdin.writeline ")>""" & retFilePath & ".output"" 2>&1"
            ocmd.StdIn.WriteLine "cd.>""" & retFilePath & ".finish"" 2>&1"
            'ocmd.StdIn.WriteLine "cd.>""" & retFilePath & """"

            'wait for finish
            While fso.FileExists(retFilePath & ".finish") = False
                WScript.Sleep(500)
            Wend
                WScript.Sleep(500)
            fso.DeleteFile retFilePath & ".finish", True
            'print output
		if fso.FileExists(retFilePath & ".output") then
            Set retFile = fso.OpenTextFile(retFilePath & ".output", ForReading, False)
            response = retFile.ReadAll()
            retFile.Close()
		set retFile=Nothing
            fso.DeleteFile retFilePath & ".output", True
		else
            ...
            retFile.Close()
            Set retFile = Nothing
	    'ocmd.stdin.writeline "(cd.)>""" & retFilePath & ".output"" 2>&1"
        Loop
